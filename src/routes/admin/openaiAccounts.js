...
function getDefaultOpenAIProxyConfig() {
  const raw = process.env.OPENAI_PROXY_CONFIG
  if (!raw) return null
...
router.post('/generate-auth-url', authenticateAdmin, async (req, res) => {
  try {
    ...
    const effectiveProxy = proxy || getDefaultOpenAIProxyConfig() || null
...
router.post('/exchange-code', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    // 从 Redis 获取会话数据
    sessionData = await redis.getOAuthSession(sessionId)
    if (!sessionData) {
      return res.status(400).json({
        success: false,
        message: '会话已过期或无效'
    ...
    // 配置代理（如果有）
    const effectiveProxy = sessionData.proxy || getDefaultOpenAIProxyConfig() || null
    ...
    logger.error('OpenAI OAuth token exchange failed:', {
      sessionId,
      codeLength: typeof code === 'string' ? code.length : 0,
      hasProxy: !!(sessionData?.proxy || process.env.OPENAI_PROXY_CONFIG),
      axios: summarizeAxiosError(error)
    })
    return res.status(500).json({
      success: false,
      message: '交换授权码失败',
...
router.post('/', authenticateAdmin, async (req, res) => {
  try {
    ...
    const accountData = {
      ...
      proxy: proxy || getDefaultOpenAIProxyConfig() || null,
      isActive: true,
      schedulable: true
    }
