...
router.post('/exchange-code', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    if (requestProxy) {
      ...
      logger.info(
        `Using proxy from request body: ${ProxyHelper.maskProxyInfo(proxyConfig)}`
      )
    }
    ...
    logger.error('❌ Failed to exchange Gemini authorization code:', {
      message: error?.message,
      code: error?.code,
      status: error?.response?.status,
      responseData: error?.response?.data,
      sessionId: req.body?.sessionId,
      codeLength: typeof req.body?.code === 'string' ? req.body.code.length : 0,
