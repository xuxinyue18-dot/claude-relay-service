...
const OAUTH_PROVIDERS = {
  [OAUTH_PROVIDER_GEMINI_CLI]: {
    // Gemini CLI OAuth 配置（通过环境变量注入，避免在仓库内硬编码敏感信息）
...
if (!process.env.GEMINI_OAUTH_CLIENT_ID || !process.env.GEMINI_OAUTH_CLIENT_SECRET) {
  logger.warn('⚠️ Gemini OAuth 环境变量未完整设置：GEMINI_OAUTH_CLIENT_ID / GEMINI_OAUTH_CLIENT_SECRET')
}
if (!process.env.ANTIGRAVITY_OAUTH_CLIENT_ID || !process.env.ANTIGRAVITY_OAUTH_CLIENT_SECRET) {
  logger.warn(
    '⚠️ Antigravity OAuth 环境变量未完整设置：ANTIGRAVITY_OAUTH_CLIENT_ID / ANTIGRAVITY_OAUTH_CLIENT_SECRET'
  )
}
