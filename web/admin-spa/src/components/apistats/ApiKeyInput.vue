<template>
  <div class="api-input-wide-card mb-8 rounded-3xl p-6 shadow-xl">
    ...
    <div class="mx-auto max-w-4xl">
      ...
      <div
        v-if="multiKeyMode"
        class="mt-2 rounded-lg bg-primary-50 p-3 text-sm text-primary-700 dark:bg-primary-900/20 dark:text-primary-400"
      >
...
.btn-primary:hover:not(:disabled) {
  transform: translateY(-1px);
  box-shadow:
    0 20px 25px -5px var(--shadow-primary-strong),
    0 10px 10px -5px var(--shadow-primary-soft);
}
...
.mode-switch-btn.active {
  color: white;
  background: linear-gradient(135deg, var(--primary-color) 0%, var(--secondary-color) 100%);
  box-shadow: 0 2px 4px var(--shadow-primary-soft);
}

.mode-switch-btn.active:hover {
  box-shadow: 0 4px 6px var(--shadow-primary-strong);
}
