  ...
  <div class="space-y-6 md:space-y-8">
    ...
    >
      ...
      <div class="card-section">
        <SectionHeader
          :icon="multiKeyMode ? 'fas fa-layer-group' : 'fas fa-info-circle'"
          :title="multiKeyMode ? '批量查询概要' : 'API Key 信息'"
          :variant="multiKeyMode ? 'magenta' : 'primary'"
        />
        ...
        <div v-if="multiKeyMode && aggregatedStats" class="info-grid">
          ...
          <div class="info-item">
            ...
            <p class="info-value text-secondary-600 dark:text-secondary-300">
      ...
      <!-- 使用统计概览 -->
      <div class="card-section">
        <SectionHeader icon="fas fa-chart-bar" title="使用统计概览" variant="success">
          <template #actions>
            <Badge variant="neutral">{{ statsPeriod === 'daily' ? '今日' : '本月' }}</Badge>
          </template>
        </SectionHeader>
        <div class="metric-grid">
          ...
          <div class="metric-card">
            <p class="metric-value text-primary-600 dark:text-primary-300">
              {{ formatNumber(currentPeriodData.allTokens) }}
            </p>
            <p class="metric-label">{{ statsPeriod === 'daily' ? '今日' : '本月' }}Token 数</p>
          </div>
          <div class="metric-card">
            <p class="metric-value text-magenta-600 dark:text-magenta-300">
    ...
    <div v-if="!multiKeyMode && boundAccountList.length > 0" class="card-section">
      <SectionHeader icon="fas fa-plug" title="专属账号运行状态" variant="secondary">
        <template #actions>
          <Badge variant="accent">实时更新</Badge>
        </template>
      </SectionHeader>
...
const getSessionProgressBarClass = (status, account) => {
  if (!status) return 'bg-gradient-to-r from-primary-500 to-secondary-500'
...
.icon-claude {
  @apply bg-gradient-to-br from-magenta-500 to-secondary-600;
}

.icon-openai {
  @apply bg-gradient-to-br from-accent-500 to-secondary-500;
}
...
.tag-indigo {
  @apply bg-secondary-100 text-secondary-600 dark:bg-secondary-500/20 dark:text-secondary-200;
}

.tag-blue {
  @apply bg-primary-100 text-primary-600 dark:bg-primary-500/20 dark:text-primary-200;
}
