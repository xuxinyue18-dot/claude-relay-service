<template>
  <div class="mb-4 sm:mb-6">
    <!-- 移动端下拉选择器 -->
    <div class="glass block rounded-xl p-2 sm:hidden">
      <select
        class="w-full rounded-lg bg-white/90 px-4 py-3 font-semibold text-gray-700 focus:outline-none focus:ring-2 focus:ring-[color:var(--primary-color)] dark:bg-gray-900/60 dark:text-gray-200"
        :value="activeTab"
    ...
    <!-- 桌面端标签栏 -->
    <div class="glass hidden flex-wrap gap-2 rounded-2xl p-2 sm:flex">
      <button
        v-for="tab in tabs"
        :key="tab.key"
        :class="[
