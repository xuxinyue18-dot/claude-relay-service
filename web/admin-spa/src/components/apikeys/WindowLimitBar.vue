<template>
  <div class="w-full space-y-1">
    <!-- 时间窗口进度条 -->
    <div
      class="relative h-7 w-full overflow-hidden rounded-md border border-opacity-20 bg-gradient-to-r from-primary-50 to-accent-100 dark:from-primary-950/30 dark:to-accent-900/30"
    >
      <!-- 时间进度条背景 -->
      <div
        class="absolute inset-0 h-full bg-gradient-to-r from-primary-500 to-accent-500 opacity-20 transition-all duration-1000"
      ...
      <div class="relative z-10 flex h-full items-center justify-between px-2">
        <div class="flex items-center gap-1.5">
          <i class="fas fa-clock text-xs text-primary-600 dark:text-primary-400" />
    ...
    <div v-if="costLimit > 0 || requestLimit > 0" class="flex gap-1">
      ...
      <div
        v-if="requestLimit > 0"
        class="relative h-6 overflow-hidden rounded-md border border-opacity-20 bg-gradient-to-r from-magenta-50 to-secondary-100 dark:from-magenta-950/30 dark:to-secondary-900/30"
...
const getRequestProgressBarClass = () => {
  ...
  if (p >= 90) {
    ...
    return 'bg-gradient-to-r from-magenta-500 to-secondary-600'
  }
}
