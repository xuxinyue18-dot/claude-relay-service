  ...
  <div>
    ...
    >
      <StatCard
        ...
        variant="primary"
      >
        <template #subtitle>活跃: {{ dashboardData.activeApiKeys || 0 }}</template>
      </StatCard>

      <StatCard icon="fas fa-user-circle" subtitle="" title="服务账户" variant="success">
        <template #value>
          <div class="flex flex-wrap items-baseline gap-x-2">
            <span>{{ dashboardData.totalAccounts }}</span>
            <div v-if="dashboardData.accountsByPlatform" class="flex items-center gap-2">
              ...
              >
                <i class="fas fa-brain text-xs text-primary-600" />
              ...
              >
                <i class="fas fa-terminal text-xs text-secondary-600" />
                <span class="text-xs font-medium text-gray-700 dark:text-gray-300">{{
              ...
              >
                <i class="fab fa-microsoft text-xs text-primary-600" />
                <span class="text-xs font-medium text-gray-700 dark:text-gray-300">{{
        ...
        <template #subtitle>
          <span>正常: {{ dashboardData.normalAccounts || 0 }}</span>
          <span v-if="dashboardData.abnormalAccounts > 0" class="text-red-600">
            | 异常: {{ dashboardData.abnormalAccounts }}
          </span>
          <span v-if="dashboardData.pausedAccounts > 0" class="text-gray-600 dark:text-gray-400">
      ...
      <StatCard
        ...
        variant="magenta"
      >
        <template #subtitle>总请求: {{ formatNumber(dashboardData.totalRequests || 0) }}</template>
      </StatCard>

      <StatCard icon="fas fa-heartbeat" subtitle="" title="系统状态" variant="warning">
        <template #value>
          <span class="text-green-600">{{ dashboardData.systemStatus }}</span>
        </template>
        <template #subtitle>运行时间: {{ formattedUptime }}</template>
      </StatCard>
    </div>
    ...
    <div
      class="mb-4 grid grid-cols-1 gap-3 sm:mb-6 sm:grid-cols-2 sm:gap-4 md:mb-8 md:gap-6 lg:grid-cols-4"
    >
      <StatCard icon="fas fa-coins" subtitle="" title="今日Token" variant="secondary">
        <template #value>
        ...
        <div class="text-xs text-gray-500 dark:text-gray-400">
          <div class="flex flex-wrap items-center justify-between gap-x-4">
            ...
            <span v-if="(dashboardData.todayCacheCreateTokens || 0) > 0" class="text-magenta-600">
              缓存创建:
              <span class="font-medium">{{
                formatNumber(dashboardData.todayCacheCreateTokens || 0)
              }}</span>
            </span>
            <span v-if="(dashboardData.todayCacheReadTokens || 0) > 0" class="text-magenta-600">
              缓存读取:
              <span class="font-medium">{{
      ...
      <StatCard icon="fas fa-database" subtitle="" title="总Token消耗" variant="success">
        <template #value>
          <div class="flex flex-wrap items-baseline gap-2">
        ...
        <div class="text-xs text-gray-500 dark:text-gray-400">
          <div class="flex flex-wrap items-center justify-between gap-x-4">
            ...
            <span v-if="(dashboardData.totalCacheCreateTokens || 0) > 0" class="text-magenta-600">
              缓存创建:
              <span class="font-medium">{{
                formatNumber(dashboardData.totalCacheCreateTokens || 0)
              }}</span>
            </span>
            <span v-if="(dashboardData.totalCacheReadTokens || 0) > 0" class="text-magenta-600">
              缓存读取:
              <span class="font-medium">{{
      ...
      >
        <template #value>
          <span class="text-orange-600">{{ dashboardData.realtimeRPM || 0 }}</span>
        </template>
        <template #subtitle>
          <span>每分钟请求数</span>
      ...
      >
        <template #value>
          <span class="text-rose-600">{{ formatNumber(dashboardData.realtimeTPM || 0) }}</span>
        </template>
        <template #subtitle>
          <span>每分钟Token数</span>
          <span v-if="dashboardData.isHistoricalMetrics" class="text-yellow-600">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex items-center rounded-lg bg-gray-100 px-3 py-1 dark:bg-gray-700">
              <label class="relative inline-flex cursor-pointer items-center">
                ...
                <!-- 更小的开关 -->
                <div
                  class="peer relative h-5 w-9 rounded-full bg-gray-300 transition-all duration-200 after:absolute after:left-[2px] after:top-0.5 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow-sm after:transition-transform after:duration-200 after:content-[''] peer-checked:bg-primary-500 peer-checked:after:translate-x-4 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-primary-300 dark:bg-gray-600 dark:after:bg-gray-300 dark:peer-focus:ring-primary-600"
                />
                <span
                ...
                >
                  ...
                  <span
                    ...
                    class="ml-1 font-mono text-xs text-primary-600 transition-opacity"
            ...
            <!-- 刷新按钮 -->
            <button
              class="flex items-center gap-1 rounded-md border border-gray-300 bg-white px-3 py-1 text-sm font-medium text-primary-600 shadow-sm transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:hover:bg-gray-700 sm:gap-2"
